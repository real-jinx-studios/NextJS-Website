...
export function usePriceCalculator(
  ...
  const [totalShippingTax, setTotalShippingTax] = useState(0);

  useEffect(() => {
    const totalPrice = price * quantity;
    const totalTax = totalPrice * taxRate;
    const totalDiscount = totalPrice * discountRate;
    const totalShipping = totalPrice * shippingRate;
    const totalShippingTax = totalShipping * shippingTaxRate;

    setTotalPrice(totalPrice);
    setTotalTax(totalTax);
    setTotalDiscount(totalDiscount);
    setTotalShipping(totalShipping);
    setTotalShippingTax(totalShippingTax);
  }, [price, quantity, taxRate, discountRate, shippingRate, shippingTaxRate]);

  return [totalPrice, totalTax, totalDiscount, totalShipping, totalShippingTax];
}

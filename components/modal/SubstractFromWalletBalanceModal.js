...
export default function SubstractFromWalletBalanceModal({
  setIsSubstractFromBalanceOpen,
  walletHash,
  walletName,
  tokens,
  ...
  const handleTransfer = async (e) => {
    ...
    const [data, error] = await promiseResolver(
      fetch("/api/subass/sa-wallet-transactions", {
        method: "POST",
        body: JSON.stringify({
          LoginToken: Cookies.get("uat"),
          "wallet-key": walletHash,
          tokens: amountToAdd * -1,
          source: "website",
        }),
      })
    );
    ...
    if (data.status === 400) {
      ...
      setIsSubstractFromBalanceOpen(false);
      toast.success("Successfully removed from wallet", {
        position: "bottom-right",
        autoClose: 1500,
        hideProgressBar: true,
        closeOnClick: true,
  ...
  return (
    <>
      ...
      <div className="add-tokens-wrapper">
        <p className="add-tokens-wrapper_title">substract from wallet</p>

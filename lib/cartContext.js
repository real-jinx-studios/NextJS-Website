...
export function loadStateLocal(cartId, username) {
  ...
  if (username && parsedState?.username === username) {
    if (parsedState.orderType === "duePayment") {
      let found = false;
      fetch("/api/rest/WebSite/due-payments", {
        ...
        .then((data) => {
          data.Payments.forEach((payment) => {
            if (payment.Order.OrderId === parsedState.duePaymentId) {
              found = true;
            }
          });
        })
      ...
      if (!found) {
        console.log("due payment not found");
        deleteCartState();
        return emptyState;
      }
      console.log("found");
    }
    console.log("returning parsed state");
    return parsedState;
  }
  deleteCartState();
}
